...
/*             Console.WriteLine (string.Format("lf_amp: {0:X4}", lf_amp));
            Console.WriteLine (string.Format("hf_amp: {0:X2}", hf_amp));
            Console.WriteLine (string.Format("l_f: {0:F}", l_f));
            Console.WriteLine (string.Format("hf: {0:X4}", hf));
            Console.WriteLine (string.Format("lf: {0:X2}", lf)); */
            return rumble_data;
        }
    }
        ...
        if (state > state_.NO_JOYCONS)
        {
			...
			if (rumble_obj.timed_rumble) {
				if (rumble_obj.t < 0) {
                    ...
                    rumble_obj.t -= DateTimeOffset.Now.ToUnixTimeMilliseconds();
				}
			}
        }
    }
